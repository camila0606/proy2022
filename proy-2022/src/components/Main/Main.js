...
export default class Main extends Component {
  ...
  componentDidMount() {
    for (let i = 1; i < 50; i++) {
      fetch(
        `https://api.themoviedb.org/3/movie/popular?api_key=7bdc01d41b64d4396399e9ee70b4980b&language=en-US&page=` +
          i
      )
        ...
        .then((results) => {
          ...
          if (i === 1) {
            this.setState({
              moviesToRender: this.state.moviesBackUp.slice(
                0,
                this.state.moviesToRenderNumber
              ),
            });
            console.log(
              "ESTAS MOVIES SE VAN A RENDERIZAR",
  ...
  moreCards() {
    this.setState({
      moviesToRender: this.state.moviesBackUp.slice(
        0,
        this.state.moviesToRenderNumber + 10
      ),
  ...
  search(textToSearch) {
    ...
    let newMoviesToRender = this.state.moviesBackUp.filter(
      (item) =>
        item.original_title
          .toLowerCase()
          .includes(textToSearch.target.value.toLowerCase()) === true
    );
    this.setState({
